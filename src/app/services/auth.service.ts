...
export class AuthService {
  ...
  async login(email: string, password: string) {
    return await this.afauth.signInWithEmailAndPassword(email, password).then(res => this.router.navigate(['welcome'])).catch(error => {
      switch (error.code) {
  ...
  async register(email: string, password: string) {
    return await this.afauth.createUserWithEmailAndPassword(email, password).then(res => this.router.navigate(['welcome'])).catch(error => {
      switch (error.code) {
        case 'auth/invalid-email':
